  ...
  <div>
    <div class="editor">
      ...
      <div class="slidecontainer">
        <input type="range" min="1" max="6" v-model.number="branch"  class="slider"> branch ({{branch}})
        <input type="range" min="0" max="100" v-model.number="xGap" class="slider"> x-gap ({{xGap}})
        <input type="range" min="0" max="100" v-model.number="yGap"  class="slider"> y-gap ({{yGap}})<br>
        <input type="checkbox" id="checkbox" v-model="sharpCorner">
        <label for="checkbox">Sharp corner</label>
      </div>
      <textarea v-model="dataJson" rows="30" cols="90" @change="updateMap" data-gramm="false" data-gramm_editor="false">
      </textarea>
    </div>
    <div class="diagram">
      <mindmap v-model="dataM" :edit="editable" v-if="renderComponent" :x-gap="xGap" :y-gap="yGap" :branch="branch"
               :sharp-corner="sharpCorner" :download-btn="true" :timetravel="true" :zoom="true"
      :drag="true" :center-btn="true" :fit-btn="true" :add-node-btn="true"></mindmap>
    </div>
  </div>
</template>
